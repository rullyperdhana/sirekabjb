<x-app-layout>
    <div class="max-w-[1200px] mx-auto">
        ...
        <div class="mb-8 flex items-center gap-4">
            ...
            <div>
                <h2 class="text-headline-md font-headline-md font-bold text-on-surface mb-2">Edit Data Transaksi</h2>
        ...
        <form action="{{ route('transaksi.update', $transaksi->id) }}" method="POST" class="space-y-8" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="bg-surface-container-lowest border border-outline-variant rounded shadow-sm p-10 relative">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                        ...
                        @else
                            <select id="skpd_id" name="skpd_id" required class="w-full h-10 px-3 rounded border border-outline-variant bg-surface text-body-md focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">
                                ...
                                @foreach($skpds as $skpd)
                                    <option value="{{ $skpd->id }}" {{ old('skpd_id', $transaksi->skpd_id) == $skpd->id ? 'selected' : '' }}>{{ $skpd->nama }}</option>
                                @endforeach
                            </select>
                        @endif
                    </div>
                    <div>
                        ...
                        <select id="rekening_id" name="rekening_id" required class="w-full h-10 px-3 rounded border border-outline-variant bg-surface text-body-md focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">
                            ...
                            @foreach($rekenings as $rekening)
                                <option value="{{ $rekening->id }}" data-skpd-id="{{ $rekening->skpd_id }}" {{ old('rekening_id', $transaksi->rekening_id) == $rekening->id ? 'selected' : '' }}>{{ $rekening->nama }} ({{ $rekening->nomor }})</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="number" name="periode_tahun" value="{{ old('periode_tahun', $transaksi->periode_tahun) }}" required class="w-full h-10 px-3 rounded border border-outline-variant bg-surface text-body-md focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">
                    </div>
                </div>
                        ...
                        <div class="p-6 border-r border-outline-variant space-y-6">
                            ...
                            <div class="pt-2">
                                ...
                                <div class="space-y-1 pl-4">
                                    ...
                                    <div class="relative">
                                        <span class="absolute left-3 top-1/2 -translate-y-1/2 text-on-surface-variant font-data-tabular">Rp</span>
                                        <input type="hidden" name="bku_penerimaan" id="bku_penerimaan" value="{{ old('bku_penerimaan', $transaksi->bku_penerimaan) }}">
                                        <input id="bku_penerimaan_display" class="calc-bku w-full h-10 pl-10 pr-4 border-outline-variant rounded focus:border-primary focus:ring-1 focus:ring-primary bg-surface-container-lowest font-data-tabular" type="text" data-target="bku_penerimaan" value="{{ old('bku_penerimaan', $transaksi->bku_penerimaan) }}"/>
                                    </div>
                                </div>
                            </div>
                            <div class="pt-2">
                                ...
                                <div class="space-y-1 pl-4">
                                    ...
                                    <div class="relative">
                                        <span class="absolute left-3 top-1/2 -translate-y-1/2 text-on-surface-variant font-data-tabular">Rp</span>
                                        <input type="hidden" name="bku_pengeluaran" id="bku_pengeluaran" value="{{ old('bku_pengeluaran', $transaksi->bku_pengeluaran) }}">
                                        <input id="bku_pengeluaran_display" class="calc-bku w-full h-10 pl-10 pr-4 border-outline-variant rounded focus:border-primary focus:ring-1 focus:ring-primary bg-surface-container-lowest font-data-tabular" type="text" data-target="bku_pengeluaran" value="{{ old('bku_pengeluaran', $transaksi->bku_pengeluaran) }}"/>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="p-6 space-y-6">
                            ...
                            <div class="pt-2">
                                ...
                                <div class="space-y-1 pl-4">
                                    ...
                                    <div class="relative">
                                        <span class="absolute left-3 top-1/2 -translate-y-1/2 text-on-surface-variant font-data-tabular">Rp</span>
                                        <input type="hidden" name="bank_penerimaan" id="bank_penerimaan" value="{{ old('bank_penerimaan', $transaksi->bank_penerimaan) }}">
                                        <input id="bank_penerimaan_display" class="calc-bank w-full h-10 pl-10 pr-4 border-outline-variant rounded focus:border-primary focus:ring-1 focus:ring-primary bg-surface-container-lowest font-data-tabular" type="text" data-target="bank_penerimaan" value="{{ old('bank_penerimaan', $transaksi->bank_penerimaan) }}"/>
                                    </div>
                                </div>
                            </div>
                            <div class="pt-2">
                                ...
                                <div class="space-y-1 pl-4">
                                    ...
                                    <div class="relative">
                                        <span class="absolute left-3 top-1/2 -translate-y-1/2 text-on-surface-variant font-data-tabular">Rp</span>
                                        <input type="hidden" name="bank_pengeluaran" id="bank_pengeluaran" value="{{ old('bank_pengeluaran', $transaksi->bank_pengeluaran) }}">
                                        <input id="bank_pengeluaran_display" class="calc-bank w-full h-10 pl-10 pr-4 border-outline-variant rounded focus:border-primary focus:ring-1 focus:ring-primary bg-surface-container-lowest font-data-tabular" type="text" data-target="bank_pengeluaran" value="{{ old('bank_pengeluaran', $transaksi->bank_pengeluaran) }}"/>
                                    </div>
                                </div>
                            </div>
                            <div class="pt-4 mt-6 border-t border-outline-variant">
                ...
                <div class="mb-4">
                    ...
                    <input type="date" name="tanggal_ba" required value="{{ old('tanggal_ba', $transaksi->tanggal_ba ?? date('Y-m-d')) }}" class="w-full h-10 px-3 rounded border border-outline-variant bg-surface text-body-md focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">
                </div>
                ...
                <div class="mb-8">
                    ...
                    <select name="status_verifikasi" required class="w-full h-10 px-3 rounded border border-outline-variant bg-surface text-body-md focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">
                        <option value="draft" {{ old('status_verifikasi', $transaksi->status_verifikasi) == 'draft' ? 'selected' : '' }}>Draft</option>
                        <option value="verified" {{ old('status_verifikasi', $transaksi->status_verifikasi) == 'verified' ? 'selected' : '' }}>Verified (Selesai)</option>
                    </select>
                </div>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (skpdSelect && skpdSelect.tagName === 'SELECT') {
                skpdSelect.addEventListener('change', filterRekenings);
            }
            filterRekenings();
        });
    </script>
