<x-app-layout>
    <div class="max-w-3xl mx-auto space-y-6">
        ...
        <div class="flex items-center gap-4 border-b-[3px] border-primary pb-4">
            ...
            <div>
                <h1 class="text-headline-lg font-headline-lg text-on-surface">Edit Pengguna</h1>
                <p class="text-body-md font-body-md text-on-surface-variant mt-1">Ubah data akun dan tingkat otorisasi Pengguna Alur 4-Pilar</p>
            </div>
        </div>
        ...
        <div class="bg-surface rounded-xl border border-outline-variant shadow-sm overflow-hidden p-6">
            ...
            <form action="{{ route('user.update', $user->id) }}" method="POST" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $user->name) }}" required class="w-full h-10 px-3 rounded-lg border border-outline-variant bg-surface text-body-md focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">
                </div>
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block font-label-sm text-label-sm text-on-surface font-semibold mb-1">Username (Login)</label>
                        <input type="text" name="username" value="{{ old('username', $user->username) }}" required class="w-full h-10 px-3 rounded-lg border border-outline-variant bg-surface text-body-md focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">
                    </div>

                    <div>
                        <label class="block font-label-sm text-label-sm text-on-surface font-semibold mb-1">Email Resmi</label>
                        <input type="email" name="email" value="{{ old('email', $user->email) }}" required class="w-full h-10 px-3 rounded-lg border border-outline-variant bg-surface text-body-md focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">
                    </div>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block font-label-sm text-label-sm text-on-surface font-semibold mb-1">Password Baru (Kosongkan jika tidak ingin mengubah)</label>
                        <input type="password" name="password" placeholder="Biarkan kosong jika tidak diubah" class="w-full h-10 px-3 rounded-lg border border-outline-variant bg-surface text-body-md focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">
                    </div>

                    <div>
                        <label class="block font-label-sm text-label-sm text-on-surface font-semibold mb-1">Konfirmasi Password Baru</label>
                        <input type="password" name="password_confirmation" placeholder="Ulangi password baru" class="w-full h-10 px-3 rounded-lg border border-outline-variant bg-surface text-body-md focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <select id="user_role" name="role" required onchange="handleRoleChange(this.value)" class="w-full h-11 px-3 rounded-lg border border-outline-variant bg-surface text-body-md focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all font-medium">
                        ...
                        <option value="bank" {{ old('role', $user->role) == 'bank' ? 'selected' : '' }}>
                            🏛️ Pilar 2: Pihak Bank (Verifikasi Rekening Koran & Mutasi Bank Kalsel)
                        </option>
                        <option value="konsolidator" {{ old('role', $user->role) == 'konsolidator' ? 'selected' : '' }}>
                            📑 Pilar 3: Konsolidator BPKAD (Verifikasi Teknis Kasda & Checklist 5 Berkas)
                        </option>
                        <option value="inspektorat" {{ old('role', $user->role) == 'inspektorat' ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="status" class="w-full h-10 px-3 rounded-lg border border-outline-variant bg-surface text-body-md focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">
                        <option value="1" {{ old('status', $user->status) == '1' ? 'selected' : '' }}>Aktif (Dapat digunakan login)</option>
                        <option value="0" {{ old('status', $user->status) == '0' ? 'selected' : '' }}>Non-Aktif (Akses login ditangguhkan)</option>
                    </select>
                </div>

                <div class="pt-4 flex justify-end gap-3">
                    <a href="{{ route('user.index') }}" class="px-5 py-2.5 rounded-lg border border-outline-variant text-on-surface hover:bg-surface-container-low transition-colors font-label-sm text-label-sm font-semibold">Batal</a>
                    <button type="submit" class="bg-primary hover:bg-primary-container text-on-primary hover:text-on-primary-container px-5 py-2.5 rounded-lg font-label-sm text-label-sm font-semibold shadow-sm transition-colors">Perbarui Pengguna</button>
                </div>
            </form>
        </div>
    </div>
